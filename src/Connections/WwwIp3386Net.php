...
class WwwIp3386Net implements IConnection
{
    private $pageURL;
    ...
    public function addresses(): array
    {
        ...
        $addresses = [];
        pq("#list table tbody tr")->each(function ($item) use (&$addresses) {
            $ip = pq($item)->find('td:eq(0)')->text();
            $port = pq($item)->find('td:eq(1)')->text();
