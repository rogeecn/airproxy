...
class WwwData5uCom implements IConnection
{
    private $pageURL;
    ...
    public function addresses(): array
    {
        ...
        $addresses = [];
        pq(".wlist ul.l2")->each(function ($item) use (&$addresses) {

            $ip = trim(pq($item)->find('li:eq(0)')->text());
            $port = trim(pq($item)->find('li:eq(1)')->text());
